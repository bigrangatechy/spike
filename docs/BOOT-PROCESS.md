...
│   ├── 00_header                  → Standard GRUB header
│   ├── 05_debian_theme            → Debian/Ubuntu theme hook
│   ├── 10_linux                   → Main boot entry generation
│   ├── 20_spike                   → Spike custom entries
│   └── 30_os-prober               → DISABLED (no dual boot)
...
GRUB_CMDLINE_LINUX_DEFAULT="quiet splash zswap.enabled=0 transparent_hugepage=madvise"
GRUB_CMDLINE_LINUX=""
# Disable os-prober (no dual boot scanning)
GRUB_DISABLE_OS_PROBER=true
# Terminal
GRUB_TERMINAL_OUTPUT=gfxterm
GRUB_FONT=/boot/grub/themes/spike/font.pf2
...
| `transparent_hugepage=madvise` | Use THP only for `madvise()` callers (prevents THP bloat) |
...
blacklist snd_hda_codec_realtek  *# No Realtek audio codec (using Intel SST)
```
...
│   ├── XDG_CURRENT_DESKTOP=Spike
│   ├── QT_QPA_PLATFORM=wayland
│   ├── WAYLAND_DISPLAY=wayland-0
│   ├── SDL_VIDEODRIVER=wayland
│   ├── MOZ_ENABLE_WAYLAND=1
│   └── LIBVA_DRIVER_NAME=[iHD|radeonsi|nvidia]
...
│   ├── kwin_wayland --noxwayland (native Wayland, XWayland loaded on demand)
...
│   │   ├── Restore tray badge from last_read_id
...
```
├── org.kde.akonadi_*            → Akonadi (stripped, not installed)
├── org.kde.baloo_*              → Baloo (stripped, not installed)
...
# GPU acceleration
LIBVA_DRIVER_NAME=iHD          *# (or radeonsi for AMD, nvidia for NVIDIA)
LIBGL_ALWAYS_SOFTWARE=0         *# Never force software rendering
# Firefox tuning (applied to Flatpak too)
MOZ_DISABLE_AV1=1              *# Disable AV1 (force VP9 on Gemini Lake)
# Qt
QT_QPA_PLATFORMTHEME=qt5ct     *# Use qt5ct for app styling
QT_AUTO_SCREEN_SCALE_FACTOR=1  *# Automatic HiDPI scaling
QT_SCALE_FACTOR=1              *# Default scale (adjustable in Settings)
# Paths
...
# Locale
LANG=en_US.UTF-8               *# (set during install)
LC_TIME=en_US.UTF-8            *# (follows locale)
# Spike-specific
SPIKE_VARIANT=standard         *# (or "plus")
