...
│  Hex: #6d4aff                                               │
│  RGB: 109, 74, 255                                           │
│  Usage: Primary brand color, buttons, selections, highlights │
│                                                              │
│  ██████████████████████████████████████████████████████████  │
│                                                              │
│  SPIKE CYAN                                                  │
│  Hex: #00bcd4                                                │
│  RGB: 0, 188, 212                                            │
│  Usage: Accent color, borders, secondary highlights, links   │
│                                                              │
│  ██████████████████████████████████████████████████████████  │
│                                                              │
└──────────────────────────────────────────────────────────────┘
...
│  Hex: #1a1a2e                                                │
│  RGB: 26, 26, 46                                             │
│  Usage: Backgrounds (GRUB, SDDM, Plymouth, wallpapers)       │
│                                                              │
│  PANEL SURFACE                                               │
│  Hex: #222236                                                │
│  RGB: 34, 34, 54                                             │
│  Usage: Desktop panel background, applet popouts              │
│                                                              │
│  CARD SURFACE                                                │
│  Hex: #2a2a4a                                                │
│  RGB: 42, 42, 74                                             │
│  Usage: Dialog backgrounds, settings cards, notification cards│
│                                                              │
│  ELEVATED SURFACE                                             │
│  Hex: #33335a                                                │
│  RGB: 51, 51, 90                                             │
│  Usage: Hovered items, active tabs, dropdown menus            │
│                                                              │
│  ██████████████████████████████████████████████████████████  │
│                                                              │
└──────────────────────────────────────────────────────────────┘
...
│  Hex: #e0e0e0                                                │
│  RGB: 224, 224, 224                                          │
│  Usage: Body text, labels, titles                             │
│                                                              │
│  SECONDARY TEXT                                              │
│  Hex: #a0a0b8                                                │
│  RGB: 160, 160, 184                                          │
│  Usage: Descriptions, hints, placeholder text, subtitles      │
│                                                              │
│  DISABLED TEXT                                               │
│  Hex: #606078                                                │
│  RGB: 96, 96, 120                                            │
│  Usage: Disabled controls, locked settings, greyed items      │
│                                                              │
│  ██████████████████████████████████████████████████████████  │
│                                                              │
└──────────────────────────────────────────────────────────────┘
...
│  Hex: #4caf50                                                │
│  RGB: 76, 175, 80                                            │
│  Usage: "Up to date", "Connected", "Installed", green status  │
│                                                              │
│  WARNING                                                     │
│  Hex: #ff9800                                                │
│  RGB: 255, 152, 0                                            │
│  Usage: "Maintenance phase", "Battery low", caution banners   │
│                                                              │
│  ERROR / CRITICAL                                            │
│  Hex: #f44336                                                │
│  RGB: 244, 67, 54                                            │
│  Usage: "Update failed", "Disk full", "Error", red status     │
│                                                              │
│  INFO                                                        │
│  Hex: #2196f3                                                │
│  RGB: 33, 150, 243                                           │
...
Usage: All UI text, documentation, website, installer, settings,
       panel, launcher, notifications, dialogs
...
Usage: Terminal (Konsole), code blocks in documentation,
       system logs, diagnostic output
...
1. Noto Sans is the ONLY sans-serif font in Spike
   ...
   └── Applications may bundle their own fonts (Firefox, LibreOffice)
       but system UI uses Noto Sans exclusively
...
├── Used on: website header, installer splash, documentation header,
│            release announcements
└── Minimum size: 120px wide (dog + text legible)

Variant 2: EMBLEM (dog only, no text)
├── Dog illustration only, no wordmark
├── Used on: GRUB menu (top center), Plymouth splash (center),
│            SDDM login screen (center), app icon, favicon,
│            panel launcher button, ISO file icon
├── Minimum size: 24px (simplified rendering at small sizes)
└── Maximum size: no limit (vector)

Variant 3: WORDMARK (text only, no dog)
├── "Spike" text only (Noto Sans Bold)
├── Used on: boot text (where graphics unavailable), CLI tools,
│            documentation footers, package names
└── Color: #e0e0e0 on dark surfaces, #1a1a2e on light surfaces

Variant 4: MONOCHROME (single color)
├── Dog silhouette in a single color
├── Used on: etched/engraved surfaces, single-color print,
│            GRUB text mode (if needed)
└── Color: adapts to background (white on dark, black on light)
...
Minimum sizes:
├── Full logo: 120px wide (digital), 3cm wide (print)
├── Emblem: 24px (digital), 0.8cm (print)
├── Wordmark: 48px wide (digital), 1.5cm (print)
└── Monochrome: 16px (digital), 0.5cm (print)
...
├── Used on: documentation footer (🐕 BigRangaTech), website footer,
│            about page, copyright notices
├── Minimum size: 48px wide
└── No standalone BigRangaTech emblem in Spike UI
...
└── Windows (KWin decorations): 0px (sharp corners for windows)
    ├── Sharp window corners distinguish windows from Spike Shell UI
    └── KWin window decoration: thin cyan border, no rounded corners
...
└── No drop shadows on UI elements (flat design language)
    └── Exception: floating panels and popouts have a subtle
        dark shadow (0px offset, 4px blur, #000000 at 40% opacity)
        to create depth from the desktop background
...
├── Active: darken red by 10% (#d32f2f)
├── Disabled: gray (#606078), text #a0a0b8
...
├── Title bar:
│   ├── Height: 32px (Standard) / 36px (Plus)
│   ├── Background: Panel Surface (#222236)
...
│   └── Design choice: cleaner appearance, saves horizontal space
└── Active vs inactive:
...
├── Circuit trace pattern (more dense than background motif)
├── Subtle purple (#6d4aff) glow in lower-right corner
├── Subtle cyan (#00bcd4) glow in upper-left corner
├── No Spike emblem (wallpaper is ambient, not branding)
...
├── Pure Dark Base (#1a1a2e)
├── No pattern, no gradient, no glow
├── For users who want maximum minimalism
├── Resolution: 3840×2160
└── File: /usr/share/spike/wallpapers/solid-dark.png

Wallpaper 3: "Spike Gradient"
├── Vertical gradient: Dark Base (#1a1a2e) at top →
│   slightly lighter (#2a2a4a) at bottom
├── Circuit trace pattern at very low opacity (5%)
├── Subtle, nearly solid
├── Resolution: 3840×2160
...
├── Spike dog emblem in lower-right corner, 20% opacity
├── Circuit trace pattern (sparse)
├── The only wallpaper featuring the Spike dog
├── Resolution: 3840×2160
...
└── Mixed approach: custom where it matters, Breeze where it doesn't
...
│   └── Night light: moon
├── Custom-designed for consistency
└── Monochrome to match panel aesthetic

Folder icons:
├── Style: folder shape with Spike color accent
├── Background: Card Surface (#2a2a4a)
├── Tab/accent: Spike Purple (#6d4aff)
├── Border: 1px Elevated Surface
├── Size: 48x48, 32x32, 22x22, 16x16
├── Main folders use accent colors:
...
├── Color-coded folders help non-technical users identify locations
└── Standard Breeze folders used for non-main folders
...
Custom cursor design (future consideration):
├── A Spike-branded cursor is possible but low priority
├── Breeze cursor is functional and recognizable
└── Only customize if a clear benefit is identified
...
Sound inventory:
├── notification.ogg          → Standard notification received
│   ├── Soft two-tone chime (descending, purple-feeling)
│   ├── Very brief (0.3s)
│   └── Volume: moderate (does not override user volume)
├── notification-critical.ogg → Critical/urgent notification
│   ├── Slightly sharper two-tone (ascending, urgent)
│   ├── Slightly louder (0.4s)
│   └── Only for critical urgency (not overused)
├── volume-change.ogg         → Volume slider adjustment
│   ├── Tiny tick (0.05s)
│   ├── Plays only when adjusting via slider, not function keys
│   └── Very quiet
├── trash-empty.ogg           → Trash emptied
│   ├── Soft crinkle/paper sound (0.3s)
│   └── Satisfying but not loud
├── startup.ogg               → Desktop ready (played at end of boot)
│   ├── Soft ascending chime (0.5s)
│   ├── Plays once when spike-session completes startup
│   ├── Can be disabled in Settings → Sound → "Play startup sound"
│   └── NOT a boot jingle (not during Plymouth — after desktop is ready)
├── shutdown.ogg              → System shutting down
│   ├── Soft descending chime (0.5s)
│   ├── Plays when shutdown initiated (before teardown)
│   └── Can be disabled in Settings → Sound → "Play shutdown sound"
└── error.ogg                 → Error dialog appears
    ├── Soft low buzz (0.2s)
    ├── Not harsh or alarming
    └── Can be disabled in Settings → Sound → "Play error sound"
...
Spike Standard (minimal animation):
├── Button hover: instant color change (no transition)
├── Button press: instant color change
├── Toggle switch: instant position change
├── Window open: instant (no animation)
├── Window close: instant (no animation)
├── Window minimize: instant (disappears)
├── Menu expand: instant (appears)
├── Notification popup: instant (appears)
├── Panel show/hide: instant
├── Launcher open: instant
├── Workspace switch: instant
└── Slider drag: follows pointer (no smoothing)

Spike Plus (animated, tasteful):
├── Button hover: 150ms color transition (ease-out)
├── Button press: 100ms color transition (ease-in)
├── Toggle switch: 200ms slide (ease-in-out)
├── Window open: 150ms fade + scale from 95% to 100% (ease-out)
├── Window close: 150ms fade + scale to 95% (ease-in)
├── Window minimize: 200ms scale to 50% + fade (ease-in)
├── Menu expand: 150ms fade + slight scale from 95% (ease-out)
├── Notification popup: 200ms slide-in from right (ease-out)
├── Panel show/hide: 200ms slide (ease-in-out)
├── Launcher open: 150ms fade + scale from 97% (ease-out)
├── Workspace switch: 200ms slide (ease-in-out)
└── Slider drag: follows pointer (no smoothing — must feel direct)
...
├── Easing: ease-out for appearances, ease-in for disappearances,
...
Bottom panel:
├── Height: 32px (Standard, default)
├── Height: 36px (Plus, default — slightly taller for animation padding)
├── Height: 40px (Large Text 1.25x)
├── Height: 48px (Large Text 1.5x)
├── Height: 64px (Large Text 2.0x)
├── Width: 100% of screen (edge to edge)
├── Position: bottom of screen (fixed, not movable — see DESIGN-DECISIONS.md)
├── Content margin: 4px from left/right edges
└── Applet spacing: 4px between adjacent applets
...
│                                                              │
│  Colors                                                      │
│  [ ] Uses only colors from the Spike palette                 │
...
│                                                              │
│  Variant Compliance                                          │
│  [ ] Looks acceptable on Spike Standard (no effects)         │
│  [ ] Looks polished on Spike Plus (with effects)             │
│  [ ] Variant differences documented (if any)                 │
│                                                              │
│  Performance                                                 │
│  [ ] No additional memory cost above baseline                │
...
What This Document Does Not Cover
    ...
    Qt stylesheet technical implementation: See dev-guide (to be written)
    QML development for SDDM: See dev-guide (to be written)
    KWin decoration configuration: See DESKTOP.md and dev-guide (to be written)
    Icon theme creation and freedesktop.org spec: See dev-guide (to be written)
    Trademark and licensing details: See GOVERNANCE.md and LICENSE (to be written)
    Sound system (PipeWire) architecture: See MULTIMEDIA.md
    Translation and internationalization: See user guide (to be written)
