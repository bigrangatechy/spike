...
After booting successfully, check:
Settings → Advanced → Boot → Boot failure counter
...
1. Check storage health:
   Settings → Advanced → Diagnostics → Storage
...
4. Check boot diagnostics:
   Settings → Advanced → Boot → View boot log
...
4. Flush DNS:
   ├── Settings → Network → Advanced → "Clear DNS Cache"
...
4. Check if updates are running:
   Settings → Software Updates
...
4. Check Flatpak repository:
   Settings → Software Sources → Flathub
...
5. If it's a specific Flatpak app:
   ├── Settings → Software Sources → Flatpak
...
1. Check if hibernate is available:
   Settings → Power
   ├── If "Hibernate" is grayed out: swap is too small
   ├── Hibernate needs swap >= RAM size
...
3. Clear caches:
   Settings → Advanced → Storage → "Clear Cache"
...
Note: Recovery mode requires physical access to the machine.
This is standard Linux behavior and is an accepted security trade-off
for a personal laptop (see RECOVERY.md )
...
1. Check retention settings:
   Settings → Notifications → Retention
...
2. Per-app notification settings:
   Settings → Notifications → Application Settings
...
Attach this file to any bug report. It contains no personal data
(no browsing history, no file contents, no passwords).
