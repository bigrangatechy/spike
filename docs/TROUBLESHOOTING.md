...
`cat /boot/grub/grubenv | grep spike\_boot\_count`


*`\# Reset manually`*

`sudo grub-editenv /boot/grub/grubenv set spike\_boot\_count=0`
```
...
`journalctl --user -u kwin\_wayland --no-pager`
...
`WAYLAND\_DEBUG=1 kwin\_wayland --replace 2\>&1 | head -50`
...
`cat /sys/class/backlight/\*/max\_brightness`


*`\# Try writing brightness directly`*

`echo 50 | sudo tee /sys/class/backlight/intel\_backlight/brightness`


*`\# Check for ACPI backlight issues (add to kernel params)`*

*`\# acpi\_backlight=video  or  acpi\_backlight=vendor`*
...
`pactl set-default-sink \<sink\_name\>`
```
...
`pactl set-source-mute \<source\_name\> 0`

`pactl set-source-volume \<source\_name\> 50%`
```
...
*`\# Look for "HARDWARE\_VIDEO\_DECODING" → should be "available"`*
...
`lsmod | grep -i -E "libinput|synaptics|i2c\_hid|psmouse"`
...
`cat /sys/class/power\_supply/BAT0/status`

`cat /sys/class/power\_supply/BAT0/current\_now`
...
`cat /sys/class/power\_supply/BAT0/uevent`
...
`loginctl unlock-session \<session\_id\>`
...
`sudo timedatectl set-timezone "America/New\_York"  *\# example`*
