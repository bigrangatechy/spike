...
`├── Kernel modules: ath5k, ath9k, ath9k\_htc, ath10k, ath10k\_pci`
...
`│   ├── Most use rtsx\_pci or rtsx\_usb (in-tree)`
...
`│   ├── Elan: elan\_i2c (in-tree, no firmware)`
...
`   │   ├── chown -R \[new\_user\]:\[new\_user\] /home/\[new\_username\]/`
...
`│   ├── Check device path for USB (via udev ID\_BUS)`
...
`└── Return struct: \{ device, type, size\_gb, model, rotational, acceptable \}`
```
...
`└── Return struct: \{ vendor, model, driver\_to\_install, va\_api\_driver \}`
```
...
`├── Return struct: \{ has\_wifi, wifi\_interface, has\_ethernet, ethernet\_interface \}`
```
