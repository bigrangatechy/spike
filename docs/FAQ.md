...
1. Download the Spike ISO from `spike.bigrangatech.com` 

2. Write it to a USB 3.0+ flash drive (minimum 8GB) using a tool like Balena Etcher or dd 

3. Plug the USB into the laptop 

4. Power on, enter the boot menu (usually F12), select the USB 

5. The live environment boots — click "Install Spike" on the desktop 

6. Follow the installer (welcome, timezone, Wi-Fi, account, variant, backup, storage, install) 

7. Reboot to your new Spike desktop 
...
4. If it fails at the same point: check storage health (the drive may be failing) 

5. If data backup was completed: your files are safe on the USB drive 
...
Yes. **Settings → Personal → Language.** Spike supports 11 languages. Changing the language requires a logout/login to take effect. See the user guide for the full language list.
...
**No.** Spike does not currently support virtual desktops or workspaces. This is a deliberate design choice — workspace switching adds cognitive load for beginner users, and the target hardware (4GB RAM) typically runs a small number of apps anyway. The panel window list shows all running applications at once.

Future consideration: If users request this in alpha feedback, workspaces may be added in a future release (possibly for Spike Plus only).

### How Do I Reduce Blue Light At Night?

Use the Night Light feature. **Settings → Display → Night Light**, or use the Night Light applet in the panel (moon/sun icon). You can set it to turn on automatically at sunset, or use a manual schedule. The color temperature decreases (warmer) to reduce eye strain during evening hours. See `DESKTOP.md` for Night Light configuration.

### Does Spike Have A Clipboard Manager?

**No.** Spike does not include a clipboard manager. Clipboard contents are stored in-memory only and are cleared on reboot, logout, or user-initiated clear. This is a deliberate privacy choice — no persistent clipboard history means no risk of sensitive data lingering in a history file.

Users who need clipboard history can install third-party clipboard managers via Discover (not included or endorsed by Spike).
...
| **Feature** | **Windows** | **Spike** |
| :-: | :-: | :-: |
| Telemetry and data collection | Extensive | No data collection at all |
| Forced updates | Yes | Security updates automatic, user controls timing |
| Hardware requirements | Requires modern hardware | Designed for 2019 Celeron hardware |
| Cost | License costs money | Free and open source (GPLv2+) |
| Terminal needed for fixes | Sometimes | Terminal never needed for user tasks |
| Dual-boot | Supported | No dual-boot |
| System requirements growth | Increases over time | Minimal, stable requirements |
| Background services | Many vendor services | Only essential services running |

### How Is Spike Different From ChromeOS Flex?

| **Feature** | **ChromeOS Flex** | **Spike** |
| :-: | :-: | :-: |
| Account requirement | Cloud-centric (requires Google account) | Fully local (no account required) |
| Primary apps | Web apps (PWA) | Native + web apps |
| Offline capability | Limited | Full offline capability |
| Telemetry | Google telemetry | Zero telemetry |
| Source code | Closed source | Open source (GPLv2+) |
| Customization | Limited | Full customization via Settings |
| Android apps | Some models | No Android app support |
| File management | Minimal local storage management | Full local file management |

### How Is Spike Different From Ubuntu / Linux Mint / Fedora?

| **Feature** | **Ubuntu / Mint / Fedora** | **Spike** |
| :-: | :-: | :-: |
| Desktop | GNOME / Cinnamon / KDE | Spike Shell (custom Qt6 desktop) |
| Purpose | General-purpose Linux | Purpose-built for old Celeron hardware |
...
| **Update Type** | **Reboot Required?** |
| :-: | :-: |
| Security updates (non-kernel) | No |
| Kernel updates | Yes — reboot to use new kernel (previous kernel retained for fallback) |
| Flatpak updates | No (restart the updated app) |
...
Migration guides are provided for major version changes. See `MIGRATION-GUIDE.md`.
...
1. Reboot — many issues resolve on restart 

2. If the system won't boot: press ESC at GRUB, select previous kernel 

3. If that works: report the kernel regression 

4. If neither kernel boots: boot from USB, use Spike Rescue to recover files 

5. See `TROUBLESHOOTING.md` for specific problem resolution 
...
2. Clear caches (thumbnail, package, Flatpak) 
...
**Primary repository:** `git.bigrangatech.com/spike/spike` (GitLab CE, self-hosted)

**GitHub mirror:** `github.com/bigrangatech/spike` (read-only)
...
- Hardware detection logic: See `HARDWARE.md` 
