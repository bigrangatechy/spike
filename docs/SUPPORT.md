...
│  (if available)           contributors            guaranteed response  │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
Documentation is the primary support channel. Most problems are
solvable without asking anyone.
...
│   ├── USER-GUIDE/             → How to use every feature (17 files)
...
├── Docs updates can be contributed by anyone (pull requests/MRs)
...
URL: forum.bigrangatech.com
Software: Discourse (modern, mobile-friendly, good search)
...
│   ├── Hardware Registry results (if user submitted)
...
URL: git.bigrangatech.com (GitLab CE Issues)
GitHub mirror: github.com (read-only issues, synced from GitLab)
...
If a chat platform is set up (Matrix, IRC, or similar):
...
Chat availability:
├── Matrix room: #spike-linux:matrix.org (if configured)
├── IRC: #spike-linux on Libera.Chat (if configured)
├──bridged between Matrix and IRC if both exist
...
├<arg_value>Explain why existing features don't solve it
...
Documentation is code — it lives in the same repository and follows
the same contribution process.
...
├<arg_value>Include your details: Spike version, variant, laptop model
...
├<arg_value>Welcome newcomers — they may be using Linux for the first time
...
All support channels operate under the Spike Code of Conduct
...
│  ───────────────────────────────────────────────────────            │
│  Forum post: 1-2 days for first response                             │
│  Bug report: 1-3 days for triage                                    │
│  Fix timeline: days to weeks (depends on cause)                     │
│  Workaround: likely provided quickly (recovery boot)                │
│  Recovery: DISASTER-RECOVERY.md covers boot failures                 │
│                                                                      │
│  High: Feature broken (can't connect Wi-Fi, no audio)              │
│  ───────────────────────────────────────────────────────            │
│  Forum post: 1-3 days for first response                             │
│  Bug report: 3-7 days for triage                                    │
│  Fix timeline: 1-4 weeks (depends on complexity)                    │
│  Workaround: attempted quickly (alternative driver, config reset)   │
│                                                                      │
│  Medium: Feature misbehaves (intermittent crash, visual glitch)    │
│  ───────────────────────────────────────────────────────            │
│  Forum post: 2-5 days for first response                             │
│  Bug report: 1-2 weeks for triage                                   │
│  Fix timeline: 4-8 weeks or next release                             │
│  Workaround: may or may not exist                                    │
│                                                                      │
│  Low: Cosmetic issue, minor annoyance                               │
│  ───────────────────────────────────────────────────────            │
│  Forum post: 3-7 days for first response                             │
│  Bug report: 2-4 weeks for triage                                    │
│  Fix timeline: next release or backlog                              │
│  Workaround: unlikely needed                                        │
│                                                                      │
│  Question: "How do I..."                                            │
│  ───────────────────────────────────────────────────────            │
│  Documentation: instant (if documented)                              │
│  Forum: 1-3 days for response                                       │
│  No bug report needed                                                │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘

These are EXPECTATIONS, not GUARANTEES.
Actual response times depend on volunteer availability.
Major bugs affecting many users get faster attention.
Obscure issues on rare hardware may take longer.
...
├── Review pull requests / merge requests (if you have expertise)
└── Moderate the forum (if invited by BigRangaTech)

Contribute code:
├── See CONTRIBUTING.md for the contribution process
├── See dev-guide/ for architecture and development docs
...
├── BigRangaTech may accept donations (check the website)
├── Donations fund: hosting costs (GitLab, forum, website)
├── Donations do NOT buy priority support (no paid support tiers)
├── No "pro" version — all features are in all versions
└── If no donation mechanism exists: that's a deliberate choice

What This Document Does Not Cover

    Code of Conduct full text: See CODE_OF_CONDUCT.md (to be written)
    Contribution process and code standards: See CONTRIBUTING.md (to be written)
    Technical architecture for developers: See dev-guide/ (to be written)
    User guide for specific features: See user-guide/ (to be written)
    Bug triage process for maintainers: See dev-guide/ (to be written)
