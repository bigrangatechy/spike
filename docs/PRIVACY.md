...
Spike's privacy philosophy can be summarized in one sentence:
  ...
  The laptop belongs to the user. The data belongs to the user.
...
Privacy is not a toggle in Spike. It is the baseline.
There is no "privacy mode" to enable because privacy is
always on. There are only permissions to grant when the
user chooses to share specific access with specific apps.
...
Zero bytes. Zero metrics. Zero telemetry. Zero analytics.
Zero crash reports. Zero usage statistics. Zero hardware surveys.

This is absolute and non-negotiable.

No "anonymous" statistics. No "opt-out" telemetry. No "we collect
data to improve your experience." None of it.

If a future contributor proposes adding telemetry of any kind,
the answer is no. This is a BDFL-level decision and is recorded
...
All of these are stripped from the ISO at build time.
...
No Spike server receives user data. No analytics endpoint exists.
...
Spike uses xdg-desktop-portal for application permission management.
This is the Wayland-native mechanism for controlling what applications
can access.
...
When an application is actively using the camera or microphone,
Spike Shell displays an indicator in the panel:
...
Spike does not pre-install Chromium, Chrome, Brave, or other browsers.
Only Firefox is included.
...
Spike does NOT recommend or endorse specific alternative browsers.
The user guide notes: "Firefox is pre-configured with strong privacy
protections. If you install a different browser, you'll need to
review its privacy settings yourself."
...
Spike does not recommend or endorse specific cloud providers.
The user guide notes: "Cloud storage is not built into Spike.
If you want cloud backup, you can install a cloud storage app
from Discover. Research the privacy policy of any cloud service
before trusting them with your files."
