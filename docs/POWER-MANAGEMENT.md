...
```
`├── Primary: /sys/class/backlight/intel\_backlight/brightness`

`├── Fallback: /sys/class/backlight/acpi\_video0/brightness`

`└── Software adjustment: /sys/class/drm/card0/device/panel\_self\_refresh`
```
...
If hardware supports thresholds (via `acpi\_call` or `tpacpi-bat`):
...
```
`├── Checks kernel module availability (acpi\_call, thinkpad\_acpi, etc.)`
