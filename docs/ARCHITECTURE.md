...
Installation flow for proprietary driver:
    ...
    NVIDIA hardware detected at boot via udev
    nouveau loads by default — display works immediately
    Post-install notification informs user of optional proprietary driver
    User navigates to Settings → Software Sources → Additional Drivers
    ubuntu-drivers tool lists compatible NVIDIA driver versions
    User selects and installs
    nouveau is blacklisted, nvidia-drm is configured
    Reboot required (gentle notification, never forced)
...
Users with NVIDIA hardware are encouraged to report their experience through the hardware registry at spike.bigrangatech.com/hardware/ so the Spike team can improve support.
Networking
...
Note: NVIDIA proprietary driver is NOT included on the ISO.
It is available as an optional post-install download.
nouveau (open-source) is included in the kernel.
