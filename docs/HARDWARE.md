...
Definition: Hardware that Spike is designed to run on as a daily driver.
            All performance baselines must be met on this hardware.
            If Spike doesn't work well here, the project has failed.
...
Definition: Hardware below Tier 1 specifications. Spike must boot,
            install, and remain functional. Performance may be
            degraded — that is acceptable. Unusable is not.
...
Definition: High-performance hardware used for development and
            regression testing. NOT representative of target users.
            Performance here must be excellent — if it's not,
            something is wrong with Spike, not the hardware.
...
├── Size check: device_size >= 128GB (reject if below)
...
3. Write to spike-config state store:
   ├── gpu_vendor: intel/amd/nvidia/hybrid
   ├── gpu_driver: i915/amdgpu/nouveau/nvidia
   ├── vaapi_driver: iHD/radeonsi/vdpau/none
   ├── gpu_mode: integrated/hybrid (hybrid only)
   └── vaapi_working: true/false (tested at first boot)

4. Apply configuration:
   ├── Module blacklist: blacklist conflicting drivers
   ├── Environment variables: LIBVA_DRIVER_NAME set in session
...
2. Classify CPU:
   ...
   │   └── Total bogomips = bogomips_per_core × core_count
   ├── Read core count:
...
├── Boot parameters:
│   ├── transparent_hugepage=madvise (prevents khugepaged overhead)
│   └── zswap.enabled=0 (conflicts with ZRAM)
├── Kernel modules:
│   ├── Load: i915, snd_hda_intel, intel_lpss (for I2C/SPI peripherals)
...
└── sysctl tunables:
    ...
    └── kernel.nmi_watchdog: 0 (saves ~1% CPU)
...
├── Kernel modules:
│   ├── Load: amdgpu (or radeon), snd_hda_intel
...
│   ├── rtl8723de: rtw88_8822be or rtl8723de (newer kernel has rtw88)
...
├── Firmware: linux-firmware (ath9k_hw, ath10k firmware)
├── Common chips:
...
5. Store in spike-config state:
   ├── wifi_vendor: intel/realtek/atheros/broadcom/none
   ├── wifi_driver: iwlwifi/rtw88_xxx/ath10k/wl/none
   ├── wifi_firmware_loaded: true/false
   ├── wifi_requires_mok: true/false (Broadcom + Secure Boot)
   └── wifi_secure_boot_compatible: true/false
...
Summary: Spike supports all Wi-Fi standards that the upstream
Linux kernel supports. No Spike-specific Wi-Fi limitations exist.

Ethernet Support

Ethernet is less critical than Wi-Fi (most target laptops use Wi-Fi),
...
└── Workaround: pcie_aspm=off (if needed, added to boot params)
...
3. Firmware loading:
   ...
   ├── Realtek: /lib/firmware/rtl_bt/ (rtlbt_*.bin)
...
5. Store in spike-config state:
   ├── bluetooth_present: true/false
   ├── bluetooth_vendor: intel/realtek/broadcom/atheros/none
   └── bluetooth_service_enabled: true/false

6. Panel applet:
   ├── If bluetooth_present: Bluetooth applet loads in panel
   ├── If bluetooth_present == false: applet does not load
...
2. Common audio chips in target hardware:
   ...
   │   ├── Driver: snd_sof (Sound Open Firmware) or legacy snd_intel_sst
...
2. Modem types:
   ...
   │   ├── Huawei: huawei_cdc_ncm driver
   │   ├── Sierra Wireless: sierra_net driver
   │   ├── ZTE: option driver + cdc_ether
   │   └── Generic: cdc_ether, qmi_wwan, cdc_ncm
...
6. Store in spike-config state:
   ├── modem_present: true/false
   ├── modem_vendor: huawei/sierra/zte/qualcomm/fibocom/none
   └── modem_service_enabled: true/false
...
└── USB device classes:
    ...
    ├── Audio (USB headsets): snd_usb_audio
...
└── Known issues:
    ...
    └── Some Elan touchpads: may need kernel parameter i2c_i801
...

{
  "hardware": {
    "cpu": {
      ...
      "threads": 2,
      "base_clock_mhz": 1100,
      "burst_clock_mhz": 2800,
      "bogomips_total": 4399,
      "architecture": "x86-64",
      "microarchitecture": "Goldmont Plus",
      "tier": 1,
      "performance_class": "tier1",
      "zram_capable": true,
      "zram_benchmark_ratio": 3.8,
      "zram_benchmark_ms": 45
    },
    "ram": {
      "total_mb": 3789,
      "type": "DDR4",
    ...
    "power": {
      ...
      "lid_switch_present": true
    },
    "modem": {
      "present": false
    },
    "tier": 1,
    "variant_recommended": "standard"
  }
}
