...
QWidget *makeVpnPage(QWidget *parent, NmClient *nm, QLabel *statusBar)
{
  ...
  auto *hint = new QLabel(
      QStringLiteral(
          ...
          "Import new profiles with nm-connection-editor or nmcli (no import wizard yet)."),
      w);
  ...
  auto load = [nm, list, status, statusBar]() {
    ...
    status->setText(vpns.isEmpty()
                        ? QStringLiteral("No VPN / WireGuard connections saved yet.")
  ...
  QObject::connect(editor, &QPushButton::clicked, w, [status]() {
    ...
    status->setText(QStringLiteral("nm-connection-editor not found — use nmcli to import."));
  });
